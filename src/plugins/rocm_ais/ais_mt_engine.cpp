...
nixlAisMtEngine::finalizePrep(std::vector<aisXferReq> &&reqs, nixlBackendReqH *&handle) const {
    ...
    // One task per request, matching GDS_MT. Each stops early if a sibling has
    // already failed so a bad transfer does not keep the pool busy.
...
nixlAisMtEngine::checkXfer(nixlBackendReqH *handle) const {
    ...
    // hipFile writes into device memory are only guaranteed visible after the
    // owning device is synchronized, so drain every GPU this request touched.
