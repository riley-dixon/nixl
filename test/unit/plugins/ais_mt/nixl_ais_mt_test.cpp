...
#define DEFAULT_TRANSFER_SIZE (10 * 1024 * 1024)
...
print_usage(const char *program_name) {
    std::cerr
        ...
        << "  -P, --skip-path-smoke   Skip the path-mode smoke test and run only the\n"
        << "                          DRAM/VRAM transfer against <directory_path>\n"
...
fill_gpu_test_pattern(void *gpu_buffer, size_t size) {
    ...
    hipError_t err = hipMemcpy(gpu_buffer, host_buffer, size, hipMemcpyHostToDevice);
    free(host_buffer);
    return err;
}
...
clear_gpu_buffer(void *gpu_buffer, size_t size) {
    return hipMemset(gpu_buffer, 0, size);
}
...
// Path-mode smoke target prefix. The harness creates its own files from it
// ("<path>.<pid>.<seq>" and a ".b" sibling), so it must name a location on a
// writable filesystem -- never a raw block device. Point it at AIS-backed
// storage to exercise the hipFile fast path:
//   NIXL_AIS_MT_PATH_MODE_FILE=/mnt/<nvme>/nixl-test/path-smoke.bin
//
// Related hipFile environment variables:
//   HIPFILE_UNSUPPORTED_FILE_SYSTEMS=true  allow filesystems without AIS support
//   HIPFILE_ALLOW_COMPAT_MODE=true         allow buffer-registration fallback
//
...
main(int argc, char *argv[]) {
    ...
    static struct option long_options[] = {{"dram", no_argument, 0, 'd'},
                                           ...
                                           {"skip-path-smoke", no_argument, 0, 'P'},
                                           {"help", no_argument, 0, 'h'},
                                           {0, 0, 0, 0}};

    while ((opt = getopt_long(argc, argv, "dvn:s:rwt:G:DPhN:", long_options, NULL)) != -1) {
        switch (opt) {
        case 'd':
            use_dram = true;
            break;
    ...
    for (i = 0; i < num_transfers; i++) {
        ...
        if (use_vram) {
            (void)hipSetDevice(devId);
            if (hipMalloc(&vram_addr[i], transfer_size) != hipSuccess) {
                std::cerr << "HIP malloc failed\n";
                goto cleanup;
            }
            if (fill_gpu_test_pattern(vram_addr[i], transfer_size) != hipSuccess) {
                std::cerr << "HIP buffer initialization failed\n";
                goto cleanup;
            }
        }
...

    {
        ...
        if (!skip_write) {
            ...
            double data_gb =
                (transfer_size * num_transfers * iterations) / (1024.0 * 1024.0 * 1024.0);
        ...
        if (!skip_read && !skip_write) {
            ...
            for (i = 0; i < num_transfers; i++) {
                if (use_vram && vram_addr[i]) {
                    int devId = i % num_gpus;
                    (void)hipSetDevice(devId);
                    if (clear_gpu_buffer(vram_addr[i], transfer_size) != hipSuccess) {
