...
static const char *DEFAULT_AIS_MT_TEST_NVME = "/dev/disk/by-id/nvme-MTR_SLC_16GB_0400000E3CBC";

static const char *
getAisMtTestNvmePath() {
    const char *env = std::getenv("NIXL_AIS_MT_TEST_NVME");
    if (env != nullptr && env[0] != '\0') {
        return env;
    }
    env = std::getenv("NIXL_HIPFILE_TEST_NVME");
    return (env != nullptr && env[0] != '\0') ? env : DEFAULT_AIS_MT_TEST_NVME;
}
...
static int
runPathModeSmoke() {
    if (access(getAisMtTestNvmePath(), F_OK) != 0) {
        std::cout << "SKIP: Microsemi test NVMe not present: " << getAisMtTestNvmePath()
                  << std::endl;
        return 0;
    }
    return nixl_test::runPathModeSmoke(
        "AIS_MTPathModeSmoke", "AIS_MT", getAisMtTestNvmePath(), 4096);
}
...
main(int argc, char *argv[]) {
    ...
    static struct option long_options[] = {{"dram", no_argument, 0, 'd'},
                                           ...
                                           {"path-smoke-only", no_argument, 0, 'P'},
